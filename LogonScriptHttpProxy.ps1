$tempDir = "C:\AKSEdgeBootstrap"
Start-Transcript -Path $tempDir\LogonScript.log
...
if ($env:HTTPS_PROXY) {
    $httpProxyServer = $env:HTTPS_PROXY
}
...
if ($AKSEEPinnedSchemaVersion -ne "useLatest") {
    ...
    $latestReleaseTag = if ($httpProxyServer) { (Invoke-WebRequest $aksEEReleasesUrl -Proxy $httpProxyServer | ConvertFrom-Json)[0].tag_name } else { (Invoke-WebRequest $aksEEReleasesUrl -Proxy $httpProxyServer | ConvertFrom-Json)[0].tag_name }
    $AKSEEReleaseDownloadUrl = "https://github.com/Azure/AKS-Edge/archive/refs/tags/$latestReleaseTag.zip"
    $output = Join-Path $tempDir "$latestReleaseTag.zip"
    if ($httpProxyServer) { Invoke-WebRequest $AKSEEReleaseDownloadUrl -OutFile $output -Proxy $httpProxyServer } else { Invoke-WebRequest $AKSEEReleaseDownloadUrl -OutFile $output }
    Expand-Archive $output -DestinationPath $tempDir -Force
    $AKSEEReleaseConfigFilePath = "$tempDir\AKS-Edge-$latestReleaseTag\tools\aksedge-config.json"
    ...
    #Remove-Item -Path "$tempDir\AKS-Edge-$latestReleaseTag" -Force -Recurse
}
...
try {
    function download2() { $ProgressPreference = "SilentlyContinue"; if ($httpProxyServer) { Invoke-WebRequest -Uri $url -OutFile $installDir\$zipFile -Proxy $httpProxyServer } else { Invoke-WebRequest -Uri $url -OutFile $installDir\$zipFile } }
    download2
}
catch {
...
# NOTE Standard workspace should be used must be refactored
...
# NOTE Must be evaluated if needed. Standard monitoring should be used
#Write-Host "`n"
...
# NOTE The server is already onbaorded in ARC
<#
Write-Host "`n"
Write-Host "Onboarding the Azure VM to Azure Arc..."

# Download the package
function download1() { $ProgressPreference = "SilentlyContinue"; if ($httpProxyServer) { Invoke-WebRequest -Uri https://aka.ms/AzureConnectedMachineAgent -OutFile AzureConnectedMachineAgent.msi -Proxy $httpProxyServer } else { Invoke-WebRequest -Uri https://aka.ms/AzureConnectedMachineAgent -OutFile AzureConnectedMachineAgent.msi } }
download1
